...
class Action {
    ...
    async sticker() {
        if (!this.isImage) {
            await this.bot.sendMessage(this.remoteJid, {
                text: `${BOT_EMOJI} ❌ Erro: Você precisa enviar uma imagem!`
            })
            return
        }
    ...
    async toImage() {
        if (!this.isSticker) {
            await this.bot.sendMessage(this.remoteJid, {
                text: `${BOT_EMOJI} ❌ Erro: Você precisa enviar uma figurinha!`
            })
            return
        }
        ...
        exec(`ffmpeg -i ${inputPath} ${outputPath}`, async (error) => {
            if (error) {
                await this.bot.sendMessage(this.remoteJid, {
                    text: `${BOT_EMOJI} ❌ Erro ao converter o sticker para imagem!`
                })
                return
            }
